= Yacht {<img src="https://secure.travis-ci.org/jmervine/yacht.png?branch=master" alt="Build Status" />}[http://travis-ci.org/jmervine/yacht]
...
=== Support for Rails style <tt>environments</tt> directory

This version of <tt>Yacht</tt> allows for inheritance from files named for their environment. It will import the file and set the file name to the root key as though it were in <tt>base.yml</tt> in that way.

    # /path/to/yacht_config_dir/environments/grandpa.yml
    :clan: McGillicuddy
    :age: 70
    :car:
      :make: oldsmobile
    ...
    # /path/to/yacht_config_dir/environments/pa.yml
    _parent: grandpa
    :age: 40
    :car:
      :year: 1980

Will be the same as the follow example:
